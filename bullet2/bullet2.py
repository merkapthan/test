...
class Animapp:
           ...
           # self.spawn_acorns_is_active=True
            def spawn_accorns_helper(): #вспомогательная внутренняя функция           
                ...
                if self.anim_is_running: 
                    self.canv.after(2000, spawn_accorns_helper)
    ...
    def win(self):
        ...
        try: 
            objects_on_canv=self.canv.find_all()
            bricks_on_canv=[]
            for object in objects_on_canv:
                
                tags=self.canv.gettags(object)
                                
                if "brick"  in tags and "rectangle" in tags: #если срабатывает, значит мы нашли brick.form\
                    bricks_on_canv.append(object)
                else:
                    continue    
            
                    #self.toggle_animation()
                    #print("Выиграл")
            if len(bricks_on_canv)==0:
                    self.toggle_animation()
                    print("Выиграл")   
                    #self.text_win=app.canv.create_text(220, 200, text='ВЫИГРАЛ', font=('Courier',10)) 
                    PicMessage.win_message()
            else:
                 print("Еще не выиграл")
        except IndexError:
            self.toggle_animation()  
            print("Выиграл но сработала интекс эррор")
            #self.text_win=app.canv.create_text(220, 200, text='ВЫИГРАЛ', font=('Courier',10)) 
            PicMessage.win_message()
    ...
    def set_bullet_type(self, bullet_type=None, left_spawn=None, height=None, right_spawn=None, down=None, on_platform=None):#метод создания снаряда
        ...
        self.new_bullet=Bullet(bullet_type, left_spawn, height, right_spawn, down, on_platform)
    ...
    def collide_bullets(self): #метод движения всех снарядов ПОКА ОН НЕ НУЖЕН и потом наверняка тоже, оставлю его для столкновений снарядов
        pass
...
class Bullet:
    def __init__(self, btype, ox1, oy1, ox2, oy2, on_platform):
        self.i=0
        self.btype=btype
        self.ox1=ox1
        self.oy1=oy1
        ...
        self.on_canvas=True
        self.on_platform=on_platform #проверка нахождения на платформе, при создании он там
        self.vx_0=0
        self.vy_0=0
        self.vx_current=0
        self.last_x0=None
...
class Acorn:
    image_path_acorn_life=PhotoImage(file='bullet_images/acorn_life.gif') 
    image_acorn_life_resized=image_path_acorn_life.subsample(15,15)
    image_path_acorn_energy=PhotoImage(file='bullet_images/acorn_energy.gif') 
    image_acorn_energy_resized=image_path_acorn_energy.subsample(15,15)
    def __init__(self, app_canv):
...
class Brick: #кубики, которые и надо выбивать
    ...
    def __init__(self, app_canv, hp, x0, y0, x1, y1):
        ...
        if self.inner_item==1:
           app_canv.itemconfig(self.form, fill="#a52a2a" ) 
    ...
    def delbrick(self, app_canv):
        del self.hp 
        if self.inner_item==1:
            app.set_bullet_type(left_spawn=self.x0+12, right_spawn=self.x1-13, height=self.y0+12, down=self.y1-13, on_platform=False)
        try:
            if  self.form:
...
class PicMessage:
    ...
    loose_pic_path=PhotoImage(file='bullet_images/loose_picture.gif')
